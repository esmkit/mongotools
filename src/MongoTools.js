...
class MongoTools {
  ...
  mongorestore(opt) {
    ...
    return new Promise(async function (resolve, reject) {
      ...
      return mt.wrapper.mongorestore(options, toRestore).then(resolve).catch(reject);
    });
  }
  ...
  rotation(opt) {
    ...
    const mt = this;
    return new Promise(async function (resolve, reject) {
      const filesystemRotationResult = await mt.fs
        .fileSystemRotation(rotationDryMode, path, ctimeMsMaxMs, cleanCount, minCount)
        .catch(reject);
