class MTOptions {
  constructor(options) {
    ...
    this.sslPEMKeyPassword =
      "sslPEMKeyPassword" in opt ? opt.sslPEMKeyPassword : process.env.MT_MONGO_SSL_PEM_KEY_PASSWORD || null; // password to decrypt the sslPEMKeyFile, if necessary
    ...
    this.excludeCollections =
      "excludeCollections" in opt ? opt.excludeCollections : process.env.MT_EXCLUDE_COLLECTIONS || null;
    ...
    this.decrypt =
      "decrypt" in opt ? opt.decrypt : (this.dumpFile && this.dumpFile.endsWith(this.defaultEncryptSuffix)) || false; // decrypt dump before restore
    ...
    this.rotationWindowsDays =
      "rotationWindowsDays" in opt ? opt.rotationWindowsDays : process.env.MT_ROTATION_WINDOWS_DAYS || 15;
    ...
    this.rotationCleanCount =
      "rotationCleanCount" in opt ? opt.rotationCleanCount : process.env.MT_ROTATION_CLEAN_COUNT || 10;
  }
