...
class MTDropbox {
    ...
    return new Promise(async function (resolve, reject) {
      ...
      dbx(
        ...
        (err, result, response) => {
          ...
          const fileNames = result.entries.filter(e => e[".tag"] === "file").map(e => e.path_lower);
  ...
  mongorestoreDownloadFromDropbox(options) {
    ...
    if (!dbxFullFilename.startsWith("/")) {
      return Promise.reject(
        `Dropbox dumpFile ${dbxFullFilename} must start with '/'. Note for Windows users: unalias node and set MSYS_NO_PATHCONV=1 may help.`
      );
    }
    return new Promise(async function (resolve, reject) {
      // create path if not exist
      if (!fs.existsSync(localPath)) {
        await fs.promises.mkdir(localPath, { recursive: true }).catch(err => {
      ...
      dbx(
        ...
        (err, result, response) => {
          ...
          resolve({ message: `dump downloaded into ${fullFileName}`, fileName, fullFileName });
        }
  ...
  backupsToClean(dbx, dryMode, deprecatedBackups, cleanCount, minCount) {
    ...
    // keep nb to clean
    var toDelete =
      deprecatedBackups.length > minCount
