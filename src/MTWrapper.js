...
class MTWrapper {
  ...
  commandConnectFromOptions(options, command = "", isRestore = false) {
    ...
    if (uri != null) {
      ...
      command +=
        " --host " +
        getOptionOrDefault(options, "host", "127.0.0.1") +
        " --port " +
        getOptionOrDefault(options, "port", 27017);
      ...
      if ("db" in options && options.db != "*" && (!isRestore || isLegacyRestoreDb())) {
        ...
        if (isOptionSet(options, "collection")) {
  ...
  commandDumpCollectionsFromOptions(options, command) {
    ...
    // deprecated includeCollections
    if (
      isOptionSet(options, "includeCollections") &&
      Array.isArray(options.includeCollections) &&
      options.includeCollections.length > 0
    ) {
  ...
  commandDumpParametersFromOptions(options, command) {
    if (isOptionSet(options, "numParallelCollections")) {
      if (!Number.isInteger(options.numParallelCollections)) {
        return reject({ error: "INVALID_OPTIONS", message: '"numParallelCollections" option must be an integer.' });
      }
  ...
  mongodump(options) {
    ...
    return new Promise(async function (resolve, reject) {
      ...
      if (!fs.existsSync(path)) {
        await fsPromise.mkdir(path, { recursive: true }).catch(err => {
